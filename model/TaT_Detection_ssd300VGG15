#Various imports
from torchvision.models.detection import ssd300_vgg16  as Net
from torchvision.models.detection import SSD300_VGG16_Weights as weights 
...
DEFAULT_PATH = Path('saved_model/ssd300')
...
def train_model(model: nn.Module, 
    ...
    ordered_dict['lr'] = lr_schedul.get_last_lr() if lr_schedul else 0
    ...
    ordered_dict['lr'] =  f"{lr_schedul.get_last_lr() if lr_schedul else 0}"
    pbar.set_postfix(ordered_dict=ordered_dict)
...
def saveTest(result: list, path_to_save: Path):
    ...
    for i, dict_r in enumerate(result):
        ...
        boxes = dict_r['boxes'] #getting the boxes from the dict
        scores = dict_r['scores'] #getting the scores from the dict
        img = cv.imread(path_img) #opening image
        
        for box, score in zip(boxes, scores):
            #drawing box around the sign
            cv.rectangle(img, (box[0], box[1]), (box[2], box[3]), (0,255,0), 2)
            #putting the name of the signal near the box
...
if __name__ == '__main__':
    ...
    EPOCH = 2_400 # defining number of epoch 1_200
    ...
    seq = v2.Compose([
        v2.Resize((300, 300), InterpolationMode.LANCZOS),
        v2.ToImage(),
    ...
    #model_weights = torch.load(Path(DEFAULT_PATH.joinpath(f'ts2/model_weights.pth')))
    
    #model.load_state_dict(torch.load(Path(DEFAULT_PATH.joinpath(f'ts{N_ESP - 1}/model_weights.pth'))))
    #model = Net()
    loader_train = DataLoader(dataset_train, batch_size=64, shuffle=True, num_workers=4, collate_fn=lambda x: tuple(zip(*x)))
    ...
    lr_schedul = StepLR(optimizer=optimizer, step_size=EPOCH, gamma=0.1)
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        epoch_loss = train_model(model, loader_train, optimizer, device, lr_schedul)
    ...
    if model_weights is None and N_ESP > 0:
        ...
        #model_weights = torch.load(Path(DEFAULT_PATH.joinpath('pesi_ok/model_weights.pth')))
    
    #loading the weights into the testing model
    model.load_state_dict(model_weights, strict=True)
    r = test_model(model, loader_test, 0.1, device) #doing testing
    ...
    plot_grapich(pandas.read_csv(save_path.joinpath('history_loss.csv'), index_col=None), 'epochs',  'SSD300VGG16(Loss)', save_path.joinpath('Loss.png'), 'epochs', 'Loss' )
